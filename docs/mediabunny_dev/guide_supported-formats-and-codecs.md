---
url: https://mediabunny.dev/guide/supported-formats-and-codecs
title: Supported formats & codecs | Mediabunny
---

# Supported formats & codecs | Mediabunny
...
The availability of the codecs provided by the WebCodecs API depends on the browser and thus cannot be guaranteed by this library. Mediabunny provides utility functions to check which codecs are able to be encoded. You can also specify custom coders to provide your own encoder/decoder implementation if the browser doesn't support the codec natively.
...
Additionally, most properties of `VideoEncodingConfig` and `AudioEncodingConfig` can be used here as well.
...
These functions are especially useful in conjunction with an output format to retrieve the best codec that is supported both by the encoder as well as the container format:
...
Encoders and decoders can be registered for all video and audio codecs supported by the library. It is not possible to add new codecs.
...
The samples passed to `onSample` **must** be sorted by increasing timestamp. This especially means if the decoder is decoding a video stream that makes use of B-frames, the decoder **must** internally hold on to these frames so it can emit them sorted by presentation timestamp. This strict sorting requirement is reset each time `flush` is called.