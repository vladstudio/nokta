---
url: https://mediabunny.dev/guide/output-formats
title: Output formats | Mediabunny
---

# Output formats | Mediabunny
...
Refer to the compatibility table to see which codecs can be used with which output format.
...
Append-only formats, in combination with `StreamTarget`, have some useful properties. They enable use with Media Source Extensions and allow for trivial streaming across the network, such as for file uploads.
...
-   `fastStart`
    ...
    -   `'in-memory'`
        ...
        This option ensures append-only writing, although all the writing happens in bulk, at the end.

    -   `'reserve'`
        Produces a file with Fast Start by reserving space at the start of the file into which the metadata will be written later. This requires knowledge about the expected length of the file beforehand. When using this option, you must set the `maximumPacketCount` field in the track metadata for all tracks.
    -   `'fragmented'`
        ...
        This option requires packet buffering.

    -   `undefined`
        The default option; it behaves like `'in-memory'` when using `BufferTarget` and like `false` otherwise.
...
The available options are the same `IsobmffOutputFormatOptions` used by MP4.
...
The available options are the same `MkvOutputFormatOptions` used by WebM.
...
-   `onPage`
    Will be called for each finalized Ogg page of the output file. The media source backing the page's track (logical bitstream) is also passed.
...
-   `xingHeader`
    ...
    When set to `false`, this option ensures append-only writing.
...
Most browsers don't support encoding MP3. Use the official `@mediabunny/mp3-encoder` package to polyfill an encoder.
