---
url: https://mediabunny.dev/guide/packets-and-samples
title: Packets & samples | Mediabunny
---

# Packets & samples | Mediabunny
...
1.  **Independence:** This library remains functional even if the WebCodecs API isn't available. Encoders and decoders can be polyfilled using custom coders, and the library can run in non-browser contexts such as Node.js.
...
When reading packets from an input file, this number specifies the relative ordering of packets. If packet A has a lower sequence number than packet B, then packet A comes first (in decode order). If two packets have the same sequence number, then they represent the same media sample.

Sequence numbers have no meaning on their own and only make sense when comparing them to other sequence numbers. If a packet has sequence number n, it does not mean that it is the nth packet of the track.
...
`EncodedPacketSink` can create *metadata-only* packets:
...
A video sample represents a single frame of video. It can be created directly from an image source, or be the result of a decoding process. Its API is modeled after VideoFrame.

### Creating video samples

Video samples have an image source constructor and a raw constructor.

The constructor of `VideoSample` is very similar to `VideoFrame`'s constructor, but uses second timestamps instead of microsecond timestamps.
...
See VideoPixelFormat for a list of pixel formats supported by WebCodecs.
...
You can convert a video sample to a WebCodecs `VideoFrame` to access additional data or to pass it to a `VideoEncoder`:
...
These methods behave like drawImage and paint the video frame at the given position with the given dimensions. This method will automatically draw the frame with the correct rotation based on its `rotation` property.
...
This method returns a valid `CanvasImageSource` you can use with drawImage.
...
To convert the data into a different pixel format, or to extract only a section of the frame, please use the `allocationSize` and `copyTo` methods on `VideoFrame` instead. Get a `VideoFrame` by running `videoSample.toVideoFrame()`.
...
An audio sample represents a section of audio data. It can be created directly from raw audio data, or be the result of a decoding process. Its API is modeled after AudioData.
...
You can convert an audio sample to a WebCodecs `AudioData` to pass it to an `AudioEncoder`:
...
You can also easily convert the audio sample into an `AudioBuffer` for use with the Web Audio API:
