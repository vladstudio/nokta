...
-   [Special identifiers and modifiers](#special-identifiers-and-modifiers)
    ...
    -   [geoDistance(lonA, latA, lonB, latB)](#geodistancelona-lata-lonb-latb)
-   [Examples](#examples)
...
-   `**@request.***`  
    ...
    -   `@request.headers.*` - the request headers as string values (ex. `@request.headers.x_token = "test"`)  
        Note: All header keys are normalized to lowercase and "-" is replaced with "\_" (for example "X-Token" is "x\_token").
    ...
    -   `@request.body.*` - the submitted body parameters (ex. `@request.body.title != ""`)  
        Note: Uploaded files are not part of the `@request.body` because they are evaluated separately (_this behavior may change in the future_).
-   `**@collection.***`
