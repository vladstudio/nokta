...
Record models provide an option to further control the fields serialization visibility in addition to the "Hidden" fields option using the [`record.hide(fieldNames...)`](about:/jsvm/interfaces/core.Record.html#hide) and [`record.unhide(fieldNames...)`](about:/jsvm/interfaces/core.Record.html#unhide) methods.
...
`onRecordUpdateRequest((e) => { // ignore for superusers if (e.hasSuperuserAuth()) { return e.next() } // overwrite the submitted "status" field value e.record.set("status", "pending") // or you can also prevent the update event by returning an error let status = e.record.get("status") if ( status != "pending" && // guest or not an editor (!e.auth || e.auth.get("role") != "editor") ) { throw new BadRequestError("Only editors can set a status different from pending") } e.next() }, "articles")`
...
Once loaded, you can access the expanded relations via [`record.expandedOne(relName)`](about:/jsvm/interfaces/core.Record.html#expandedOne) or [`record.expandedAll(relName)` methods.](about:/jsvm/interfaces/core.Record.html#expandedAll)
...
PocketBase Web APIs are fully stateless (aka. there are no sessions in the traditional sense) and an auth record is considered authenticated if the submitted request contains a valid `Authorization: TOKEN` header _(see also [Builtin auth middlewares](about:/docs/js-routing/#builtin-middlewares) and [Retrieving the current auth state from a route](about:/docs/js-routing/#retrieving-the-current-auth-state) )_ .
