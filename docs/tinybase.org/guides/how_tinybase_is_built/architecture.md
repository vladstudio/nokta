---
title: "Architecture | TinyBase"
url: https://tinybase.org/guides/how-tinybase-is-built/architecture
---
...
`gulpfile.mjs` is an important file, since it describes all the build steps for the project, many of which are described in the [Developing TinyBase](/guides/how-tinybase-is-built/developing-tinybase/) guide.
...
Most modules have a similar pattern: a single creation function (such as the [`createStore`](/api/the-essentials/creating-stores/createstore/) function in the case of the [`store`](/api/store/) module), and a major interface that the returned object conforms to (such as the [`Store`](/api/the-essentials/creating-stores/store/) interface).

As well as those, there are a set of common functions and utilities in the `common` subdirectory, implementations used by the [`persisters`](/api/persisters/) module in the `persisters` subdirectory, and hooks and components for the [`ui-react`](/api/ui-react/) module in the `ui-react` subdirectory.
...
There is more about TinyBase's testing in the following [Testing](/guides/how-tinybase-is-built/testing/) guide.
...
These build configurations are all defined in the `compileForProd` task in `gulpfile.mjs`, and instructions for importing them are in the [Importing TinyBase](/guides/the-basics/importing-tinybase/) guide.