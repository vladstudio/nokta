---
title: "Cloudflare Durable Objects | TinyBase"
url: https://tinybase.org/guides/integrations/cloudflare-durable-objects
---
...
Each Durable Object can function as a WebSocket server, so it can co-ordinate messages between multiple clients. But importantly, it also has private, transactional and strongly consistent storage attached. Combined with a TinyBase [`Store`](/api/the-essentials/creating-stores/store/) on the client, and using the built-in synchronization and persistence functionality, this gives you an full-stack way to build complex, real-time, multi-device (and even collaborative) apps.

As this guide hopefully shows, this can be done with minimal effort!

### [Getting Started](/guides/the-basics/getting-started/) with Vite

The quickest way to get started with TinyBase and Cloudflare Durable Objects is to use our [Vite template](https://github.com/tinyplex/vite-tinybase-ts-react-sync-durable-object). This includes both a client implementation (that by default connects to a demo server we provide) and a server implementation that you can instead use for your own Cloudflare installation.
...
#### Client: [Persistence](/guides/persistence/) and [Synchronization](/guides/synchronization/)

On the client, we create a simple TinyBase [`MergeableStore`](/api/mergeable-store/interfaces/mergeable/mergeablestore/) to encapsulate the data we need for the app. The following are simplified extracts of the code in the `App.tsx` file of the Vite template:
...
We also create a local [`Persister`](/api/the-essentials/persisting-stores/persister/) so that if the client goes offline, and the browser window is refreshed, changes will have been cached locally in session storage, with a key 'foo':
...
More interestingly, we also set up a [`Synchronizer`](/api/the-essentials/synchronizing-stores/synchronizer/) that connects to the Cloudflare installation, on a path that also happens to be called 'foo':
...
Those simple lines are enough to have the [`Store`](/api/the-essentials/creating-stores/store/) attempt to synchronize itself with the common Durable Object called `/foo` on the server.
...
Now we need to create the Durable Object itself. This can be as simple as simply extending TinyBase's [`WsServerDurableObject`](/api/the-essentials/synchronizing-stores/wsserverdurableobject/) class:
...
This sets up synchronization between any clients that connect to this common `/foo` path so that their [`Store`](/api/the-essentials/creating-stores/store/) data stays in sync.
...
Stores the entire [`Store`](/api/the-essentials/creating-stores/store/) as JSON in a single database row. This is efficient for smaller stores and uses fewer database writes, but may hit Cloudflare's 2MB row limit for very large stores.
...
In this method, all we need to do is create a [`MergeableStore`](/api/mergeable-store/interfaces/mergeable/mergeablestore/) (that will reside in the Durable Object memory whenever it is running and not hibernated), and indicate how it will be persisted. The [`DurableObjectSqlStoragePersister`](/api/persister-durable-object-sql-storage/interfaces/persister/durableobjectsqlstoragepersister/) is dedicated to storing a TinyBase [`Store`](/api/the-essentials/creating-stores/store/) in Durable Object SQLite storage, while the [`DurableObjectStoragePersister`](/api/persister-durable-object-storage/interfaces/persister/durableobjectstoragepersister/) uses the legacy key-value storage.
...
Finally, the [`WsServerDurableObject`](/api/the-essentials/synchronizing-stores/wsserverdurableobject/) uses hibernation, which is a Cloudflare feature to minimize the amount of memory used by your Durable Object. After a small amount of time with no WebSocket activity, it will be 'hibernated' even though the WebSockets stay connected. This means that the in-memory TinyBase [`Store`](/api/the-essentials/creating-stores/store/) will be removed (which is a good thing for your Cloudflare usage!) and then re-created with the [`Persister`](/api/the-essentials/persisting-stores/persister/) when new activity arrives. This lifecycle should be transparent, but should be understood if you want to debug certain Durable Object behaviors.