---
title: "Schema-Based Typing | TinyBase"
url: https://tinybase.org/guides/schemas/schema-based-typing
---
...
In this example, the store is known to have the [`ValuesSchema`](/api/store/type-aliases/schema/valuesschema/) provided, and all relevant methods will have type constraints accordingly, even for listeners:
...
### Getting the Typed [`Store`](/api/the-essentials/creating-stores/store/)

Only the [`setSchema`](/api/store/interfaces/store/store/methods/setter/setschema/) method, [`setTablesSchema`](/api/store/interfaces/store/store/methods/setter/settablesschema/) method, and [`setValuesSchema`](/api/store/interfaces/store/store/methods/setter/setvaluesschema/) method return a typed [`Store`](/api/the-essentials/creating-stores/store/) object. So, to benefit from the typing, ensure you assign your [`Store`](/api/the-essentials/creating-stores/store/) variable to what those methods return, rather than just the [`createStore`](/api/the-essentials/creating-stores/createstore/) function.
...
Schema-based typing for the [`ui-react`](/api/ui-react/) module is handled a little differently, due to the fact that all of the hooks and components are top level functions in the module. It would be frustrating to apply a schema to type each and every one in turn.
...
You may also need to use the `unknown as` cast when using the `omni` module, since it exports both the [`ui-react`](/api/ui-react/) module and [`ui-react-dom`](/api/ui-react-dom/) module, and you will need to distinguish them:
...
In the case that you have multiple [`Store`](/api/the-essentials/creating-stores/store/) objects with different schemas, you will need to use `WithSchemas` several times, and deconstruct each, something like this:
...
One final pattern to be aware of is that you can use the `WithSchemas` type to create a schema-based [`Store`](/api/the-essentials/creating-stores/store/) in a non-rendering React component. This allows you to create a [`Store`](/api/the-essentials/creating-stores/store/) with a schema, define its persistence behavior, publish it into the context, and even expose domain-specific hooks - all in a single self-contained file.
...
The `MyStore` component renders `null` so it doesn't appear visually, but it is responsible for creating the [`Store`](/api/the-essentials/creating-stores/store/) and making it available to the rest of the app via the Provider context that wraps them both.

The `MyStore.tsx` file might look something like this. (In this simplified case, we are just typing key values in our [`Store`](/api/the-essentials/creating-stores/store/) but of course this would work for a tabular [`Store`](/api/the-essentials/creating-stores/store/) too.)
...
This means that the rest of the app can use [`Values`](/api/store/type-aliases/store/values/) from the [`Store`](/api/the-essentials/creating-stores/store/) with correct types, _and_ the implementation details of the [`Store`](/api/the-essentials/creating-stores/store/) are encapsulated in the single file. For more complex applications, it really helps to keep everything about the [`Store`](/api/the-essentials/creating-stores/store/) in one place.
...
We move on to discussing more complex programmatic enforcement of your data, and for that we turn to the [Mutating Data With Listeners](/guides/schemas/mutating-data-with-listeners/) guide.